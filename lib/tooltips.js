...
const addTooltipIfOk = ({ tooltips, defaultTextStyle, time, observableIndex, valueIndex, tooltip }) => {
...
export const extractTooltips = (observables, defaultTextStyle) => {
    ...
    for (
        ...
        for (let valueIndex = 0, valuesCount = values.length; valueIndex < valuesCount; valueIndex++) {
...
export const getTooltipsModels = ({
    ...
    for (let i = 0, len = tooltips.length; i < len; i++) {
        ...
        const { observableIndex, time, text, textStyle, backgroundColor } = tooltips[i];

        const shapeCenterX = marginLeft + scale(time);
        const shapeBottomY = countsHeight + observableHeight * (observableIndex + 0.5) + shapeSize / 2;
...
export const getSvgDimensions = ({ svgWidthWithoutTooltips, svgHeightWithoutTooltips, tooltipsModels }) => {
...
export const getTooltipsToShow = (tooltips, allTooltips) =>
    allTooltips.filter(
        (tooltip) =>
            ...
            tooltips.some((t) => t.observableIndex === tooltip.observableIndex && t.valueIndex === tooltip.valueIndex),
    );
...
export const addTooltip = (observableIndex, valueIndex, tooltips, allTooltips) => {
    const tooltipToAdd = allTooltips.find(
        (tooltip) => tooltip.observableIndex === observableIndex && tooltip.valueIndex === valueIndex,
    );
...
export const removeTooltip = (observableIndex, valueIndex, tooltips) => {
    for (let i = tooltips.length - 1; i >= 0; i--) {
        ...
        if (tooltip.observableIndex === observableIndex && tooltip.valueIndex === valueIndex) {
