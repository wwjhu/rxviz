...
import { defaultMainColor, defaultObservableColor, defaultShapeColor } from './colors';

const isObservable = (value) => isPlainObject(value) && value.type === 'observable' && Array.isArray(value.values);
...
const prepareItem = ({ isObservable, isError, item, observableIndex, valueIndex, renderer }) => {
    if (isFunction(renderer)) {
        ...
        const color = isError ? {} : { color: isObservable ? defaultObservableColor : defaultShapeColor };
...
export const updateData = (observable, path, value) => {
    ...
    const dataPath = path.reduce((acc, index) => acc.concat(index, 'values'), ['values']);
...
export const getModel = ({ data, renderer, inheritMainColor = true, mergeThreshold }) => {
    let result = {
        observables: [],
        connectors: [],
    };
    ...
    while (queue.length > 0) {
        ...
        for (let i = 0, len = observable.values.length; i < len; i++) {
            ...
            if (isObservable(item)) {
                ...
                const lastValue = valueIndex === 0 ? null : resultObservable.values[valueIndex - 1];

                if (!mergeThreshold || !lastValue || value.time - lastValue.time > mergeThreshold) {
                    resultObservable.values.push(value);
                } else {
                    if (typeof lastValue.count === 'number') {
                        ...
                        /* First merge */
                        lastValue.mergedTooltip = [getTextForMergedTooltip(lastValue), getTextForMergedTooltip(value)];
