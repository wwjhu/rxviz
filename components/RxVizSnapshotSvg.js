...
export default class RxVizSnapshotSvg extends Component {
    ...
    tooltipsDimensionsAreReady({ tooltips, tooltipsDimensions }) {
        return tooltips.length === 0 || (tooltipsDimensions !== null && tooltipsDimensions.length === tooltips.length);
    }
    ...
    renderObservables(marginLeft, scale, axisWidth) {
        ...
        return observables.map((observable, observableIndex) => {
            ...
            const progressWidth = shouldRenderAxis ? (axisWidth * endTime) / timeWindow : axisWidth;
    ...
    renderTooltips(tooltipsModels) {
        ...
        return (
            <g style={tooltipsContainerStyle}>
                {tooltipsModels.map(({ x, y, width, height, text, textStyle, backgroundColor }, index) => (
                    <Tooltip
    ...
    render() {
        ...
        return (
            <svg width={svgDimensions.width} height={svgDimensions.height} style={svgStyle} ref={this.onSvgRender}>
