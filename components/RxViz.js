...
export default class RxViz extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.timeWindow !== this.props.timeWindow || nextProps.observable$ !== this.props.observable$) {
            this.setState({
                timeProgressed: 0,
                data: getEmptyObservableData(),
            });
    ...
    renderSnapshot() {
        const { name, height, timeWindow, shapeSize, renderer, inheritMainColor, mergeThreshold } = this.props;
        ...
        return (
            <Measure bounds={true} onResize={this.onResize}>
                {({ measureRef }) => (
                    ...
                    <div ref={measureRef}>{width === null ? null : this.renderSnapshot()}</div>
                )}
            </Measure>
        );
    }
