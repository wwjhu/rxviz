...
import { extractTooltips, getTooltipsToShow, addTooltip, removeTooltip } from '../lib/tooltips';
...
export default class RxVizSnapshot extends Component {
    ...
    onErrorMouseEnter = ({ observableIndex }) => {
        this.setState((state) => {
            ...
            const newTooltips = addTooltip(observableIndex, 'error', tooltips, allTooltips);
    ...
    onValueMouseEnter = ({ observableIndex, valueIndex }) => {
        this.setState((state) => {
            const { tooltips, allTooltips } = state;
            const newTooltips = addTooltip(observableIndex, valueIndex, tooltips, allTooltips);
    ...
    render() {
        const { name, width, height, timeWindow, timeProgressed, shapeSize, animate } = this.props;
        const { observables, connectors, allTooltips, tooltips, elementsToMeasure, tooltipsDimensions } = this.state;

        return (
            <div>
                <MeasureSvgElements onChange={this.onTooltipsDimensionsChange}>{elementsToMeasure}</MeasureSvgElements>
                <RxVizSnapshotSvg
                    name={name}
                    width={width}
                    height={height}
